function seqkl_stdpass(obj,varargin)
   A = varargin{1};
   ...
   debug = 1;
      ...
      % if i==1 && k==0,
         ...
         [obj.SEQKL_U(:,1:k),R] = qr(A(:,1:k),0);
         flops = flops + 4*m*k^2;
         [Ur,Sr,Vr] = svd(triu(R));
         obj.S(1:k) = diag(Sr);
         obj.SEQKL_U(:,1:k) = obj.SEQKL_U(:,1:k)*Ur;
         flops = flops + 2*m*k^2;
         obj.SEQKL_V(1:k,1:k) = Vr;
         % sort singular values
         if (isequal(obj.whch,'S')) 
            [obj.S(1:k),order] = sort(obj.S(1:k),1,'ascend');
         else
            [obj.S(1:k),order] = sort(obj.S(1:k),1,'descend');
         end
         obj.SEQKL_U(:,1:k)   = obj.SEQKL_U(:,order);
         obj.SEQKL_V(1:k,1:k) = obj.SEQKL_V(1:k,order);
